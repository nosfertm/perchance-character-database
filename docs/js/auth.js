...
const app = Vue.createApp({
    ...
    data() {
        return {
            ...
            // Theme control
            isDarkMode: true  // Default to light mode, change to true for dark mode
        };
    },
    ...
    methods: {
        ...
        async signInWithTwitter() {
            try {
                // Call Supabase auth API to sign in with Twitter
                const { error } = await supabase.auth.signInWithOAuth({
                    provider: 'twitter',
                    options: {
                        redirectTo: window.location.origin // Redirect to the same page after auth
                ...
                this.error = 'Failed to sign in with Twitter.';
                console.error(err);
            }
        },
